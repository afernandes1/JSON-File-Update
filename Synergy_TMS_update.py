...
def csvfile():
    ...
    with open(r"C:\Coty\TMS Export\tmsexport.csv", "r") as my_file:
...
def jsonfile():
    with open('C:\Coty\TMS Export\config.json') as f:
...
for values in to_csv:                               # reads every IP Address from JSON File
    ...
    if any(values in s for s in csv_IP):            # if any IP Address from JSON file is also inside the CSV File then:
        ...
        with open(r'C:\Coty\TMS Export\config.json', 'r') as file:  # opens original JSON file again as read
        ...
        with open(r'C:\Coty\TMS Export\config.json', "w") as f:     # opens JSON File as writable and saves the new configuration with the variable created above
            f.write(newText)
            output = f.close()
